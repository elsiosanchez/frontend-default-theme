...
export default defineComponent({
  ...
  setup() {
    ...
    const pointUuid = computed(() => {
      return store.getters.getPoint.uuid
    })
    ...
    const listPoint = computed(() => {
      return store.getters.getListPoint
    })
    ...
    const pointPrices = computed(() => {
      return store.getters.getPoint.priceList.name
    })
    ...
    const pointWarehouses = computed(() => {
      return store.getters.getPoint.warehouse.name
    })
    ...
    function updateListAvalibles(point) {
      store.dispatch('listAvailablePrices', point.uuid)
      store.dispatch('listAvailableWarehouse', point.uuid)
      store.dispatch('listAvailableDocumentTypes', point.uuid)
    }
